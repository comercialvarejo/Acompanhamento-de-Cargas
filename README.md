...
1. Crie um repositório novo no GitHub (por exemplo `painel-cargas`).
2. Envie estes arquivos para ele:
   ...
   git branch -M main
   git remote add origin https://github.com/SEU-USUARIO/painel-cargas.git
...
5. Espere um ou dois minutos. O endereço será:
   ...
   ```
   https://SEU-USUARIO.github.io/painel-cargas/
   ```
...
python3 scripts/painel.py exportar

# 2) edite dados/viagens.json com as cargas novas

# 3) coloca de volta no painel
python3 scripts/painel.py atualizar dados/viagens.json
...
```bash
python3 scripts/painel.py conferir              # resumo do que está publicado
python3 scripts/painel.py senha "NovaSenha123"  # troca a senha do modo admin
```
...
O status das cargas fica em um Realtime Database do Firebase
(projeto `status-cargas-varejo`), configurado no final do `index.html`.
...
- **Troque a senha antes de divulgar**, com
  `python3 scripts/painel.py senha "SuaSenhaNova"`.
...
| `dados/viagens.json` | Cópia editável das cargas, usada para atualizar |
| `scripts/painel.py` | Exporta, valida e atualiza as cargas |
...
(`painel-cargas-v1`): quando ele muda, os celulares baixam a versão nova.
O `scripts/painel.py` já sobe esse número sozinho a cada atualização — se você
